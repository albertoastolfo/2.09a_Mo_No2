...
#address = "http://128.40.160.175:5000/" 
address = "http://192.168.90.100:5000/" 

def get_shutter_state(shutter_no=1):

    #response = requests.get("http://128.40.160.162:5000/shutter/status")
    #response = requests.get("http://128.40.160.175:5000/status", json={"shutter_no": shutter_no})
    response = requests.get(address + "status", json={"shutter_no": shutter_no})
    #print(response.json())
...
def get_state():
    ...
    #response = requests.get("http://128.40.160.175:5000/status")
    ...
    vacuum = data['data']['vacuum_voltage']
    
    state.append(str(vacuum))
    
    
    #state = re.findall(r'\d+', data['data']['raw']['vacuum_voltage'])
...
def get_vacuum_from_state(state):
    
    return int(state[-1])


# Open the specified shutter (1 or 2)
def Open_Shutter(shutter_no = 2):
    ...
    if shutter_no == 1 or shutter_no == 2:
        ...
        #requests.post("http://128.40.160.175:5000/open", json={"shutter_no": shutter_no})
        requests.post(address + "open", json={"shutter_no": shutter_no})
    else:
        raise ValueError("Invalid shutter number. Use 1 or 2.")


# Close the specified shutter (1 or 2)
def Close_Shutter(shutter_no = 2):
    # Determine command based on shutter number
    if shutter_no == 1 or shutter_no == 2:
        #response = requests.post("http://128.40.160.162:5000/shutter/1/close")
        #requests.post("http://128.40.160.175:5000/close", json={"shutter_no": shutter_no})
        requests.post(address + "close", json={"shutter_no": shutter_no})
    else:
        raise ValueError("Invalid shutter number. Use 1 or 2.")
